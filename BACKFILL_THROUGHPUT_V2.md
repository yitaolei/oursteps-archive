...
Before the conservative 20-thread trial, the active `local.oursteps.historical-backfill` LaunchAgent ran at 07:00, 09:00, 11:00, 13:00, 15:00, 17:00, 19:00, and 21:00 local time with `--now --max-threads 15 --max-minutes 45`. The trial changes only the cap to 20; cadence, `--now`, the 45-minute hard stop, and all request pacing/backoff rules remain unchanged. Only the 07:00 run naturally falls inside the current robots Visit-time; later runs still depend on the pre-existing `--now` bypass. Robots-aligned scheduling remains the next structural Phase C task.
